...
if [ -d ${outDIR}/annotate_rnd4 ]; then
    echo "Folder augustus exists ... "
else
    mkdir -p ${outDIR}/annotate_rnd4
fi

cd ${outDIR}/annotate_rnd4

/usr/bin/time --verbose blastp -query ../S_flavescens.split_rnd4.all.maker.proteins.fasta \
    ...
    -out S_flavescens.split_rnd4.all.maker.proteins.blastp.txt
