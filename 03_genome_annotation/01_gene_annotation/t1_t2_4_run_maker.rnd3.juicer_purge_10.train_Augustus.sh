...
if [ -d ${outDIR}/train_augustus_rnd3 ]; then
    echo "Folder augustus exists ... "
else
    mkdir -p ${outDIR}/train_augustus_rnd3
fi

cd ${outDIR}/train_augustus_rnd3

## prepare data for gene model training
##step 1, format maker gff file
echo -ne "*** Format and filter maker gff file ***"
date
/usr/bin/time --verbose maker2zff -l 50 -x 0.2 ../S_flavescens.split_rnd3.all.gff
...
tar -zxvf S_flavescens_rnd3.tgz

ln -f -s ${outDIR}/train_augustus_rnd3/S_flavescens_rnd3 /hpcfs/apps/maker/exe/augustus/config/species/
