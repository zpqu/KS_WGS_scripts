...
if [ -d ${outDIR}/annotate_rnd1 ]; then
    echo "Folder augustus exists ... "
else
    mkdir -p ${outDIR}/annotate_rnd1
fi

cd ${outDIR}/annotate_rnd1

/usr/bin/time --verbose blastp -query ../S_flavescens.split_rnd1.all.maker.proteins.fasta \
    ...
    -out S_flavescens.split_rnd1.all.maker.proteins.blastp.txt
