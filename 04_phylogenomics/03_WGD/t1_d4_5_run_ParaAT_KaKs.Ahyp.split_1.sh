...
/usr/bin/time --verbose ParaAT.pl -h ${sname}.wgd.homologs_1 \
    -n ${genomeDIR}/cds_simple_id.fasta \
    -a ${genomeDIR}/prot_simple_id.fasta \
    -p ../proc \
    -o output_split_1 -f axt -k -g \
    >run_ParaAT_WGD.split_1.log

find -type f -wholename './output_split_1/*.kaks' -print0 | xargs -0 cat > ${sname}.wgd.kaks_1

tar -zcf output_split_1.tar.gz ./output_split_1
rm -r output_split_1
