...
/usr/bin/time --verbose ParaAT.pl -h ${sname}.wgd.homologs_5 \
    -n ${genomeDIR}/cds_simple_id.fasta \
    -a ${genomeDIR}/prot_simple_id.fasta \
    -p ../proc \
    -o output_split_5 -f axt -k -g \
    >run_ParaAT_WGD.split_5.log

find -type f -wholename './output_split_5/*.kaks' -print0 | xargs -0 cat > ${sname}.wgd.kaks_5

tar -zcf output_split_5.tar.gz ./output_split_5
rm -r output_split_5
