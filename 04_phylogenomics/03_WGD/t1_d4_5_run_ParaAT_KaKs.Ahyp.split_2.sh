...
/usr/bin/time --verbose ParaAT.pl -h ${sname}.wgd.homologs_2 \
    -n ${genomeDIR}/cds_simple_id.fasta \
    -a ${genomeDIR}/prot_simple_id.fasta \
    -p ../proc \
    -o output_split_2 -f axt -k -g \
    >run_ParaAT_WGD.split_2.log

find -type f -wholename './output_split_2/*.kaks' -print0 | xargs -0 cat > ${sname}.wgd.kaks_2

tar -zcf output_split_2.tar.gz ./output_split_2
rm -r output_split_2
