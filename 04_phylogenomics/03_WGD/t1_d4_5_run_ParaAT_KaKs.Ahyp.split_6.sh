...
/usr/bin/time --verbose ParaAT.pl -h ${sname}.wgd.homologs_6 \
    -n ${genomeDIR}/cds_simple_id.fasta \
    -a ${genomeDIR}/prot_simple_id.fasta \
    -p ../proc \
    -o output_split_6 -f axt -k -g \
    >run_ParaAT_WGD.split_6.log

find -type f -wholename './output_split_6/*.kaks' -print0 | xargs -0 cat > ${sname}.wgd.kaks_6

tar -zcf output_split_6.tar.gz ./output_split_6
rm -r output_split_6
