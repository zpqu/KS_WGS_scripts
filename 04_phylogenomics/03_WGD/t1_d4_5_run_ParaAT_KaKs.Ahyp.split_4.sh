...
/usr/bin/time --verbose ParaAT.pl -h ${sname}.wgd.homologs_4 \
    -n ${genomeDIR}/cds_simple_id.fasta \
    -a ${genomeDIR}/prot_simple_id.fasta \
    -p ../proc \
    -o output_split_4 -f axt -k -g \
    >run_ParaAT_WGD.split_4.log

find -type f -wholename './output_split_4/*.kaks' -print0 | xargs -0 cat > ${sname}.wgd.kaks_4

tar -zcf output_split_4.tar.gz ./output_split_4
rm -r output_split_4
